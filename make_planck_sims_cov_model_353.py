...
def get_autopower(map_, mask, lmax):
    return get_power(map1=map_, map2=map_, mask=mask, lmax=lmax)


def get_power(map1, map2, mask, lmax, use_pixel_weights=False):
    if mask is None:
        ...
        ps = hp.anafast(mask*(map1-mean1),
                        # mask*(map2-mean2),  # only needed for cross-spectra
                        lmax=lmax,
...
def get_ps_data(detector):
    ...
    galactic_mask = np.ones(hp.nside2npix(2048))
    galactic_mask[hp.query_strip(2048, np.radians(85), np.radians(95))] = 0
    ...
    for i in tqdm(range(N_PLANCK_SIMS)):
        ...
        new_cl = get_autopower(t_src_map, galactic_mask, lmax)
        src_cls.append(new_cl)
    ...
    np.savez(f"noise_model_{detector}GHz_diff_masked2.npz", 
             mean_ps=mean_ps, 
